...
from ..io.nd2 import read_frame_2d
from ..io import tiff


def run_convert(input_nd2: Path, pos_slice: str, time_slice: str, output: Path, *, on_progress: ProgressCallback | None = None) -> None:
    import nd2
    ...
    if on_progress:
        on_progress(0.0, f"Selected {len(pos_indices)} positions, {len(time_indices)} timepoints, {n_chan} channels, {n_z} z-slices. Total frames: {total}")
        ...
        for t_new, t_orig in enumerate(time_indices):
            for c in range(n_chan):
                for z in range(n_z):
                    ...
                    fname = f"img_channel{c:03d}_position{p_idx:03d}_time{t_new:09d}_z{z:03d}.tif"
                    tiff.write(pos_dir / fname, frame)
                    done += 1
