...
pub fn run(args: CropArgs, progress: impl Fn(f64, &str)) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pos_dir = if pos_dir_candidate.exists() {
        ...
        if p.file_name().map(|v| v.to_string_lossy().to_string())
            == Some(format!("Pos{}", args.pos))
        {
            p.to_path_buf()
        } else {
            return Err(
                format!("Position directory not found under input: Pos{}", args.pos).into(),
            );
        }
    };
    ...
    let c_to_i: HashMap<u32, usize> = channel_vals
        .iter()
        .enumerate()
        .map(|(i, v)| (*v, i))
        .collect();
    ...
    let roi_ids: Vec<i32> = bboxes
        .iter()
        .map(|b| b.crop.parse::<i32>().unwrap_or(0))
        .collect();
    let idx_roi_ids = zarr::create_array_i32(
        &roi_store,
        schema::roi_ids_path(),
        vec![roi_ids.len() as u64],
        vec![roi_ids.len() as u64],
    )?;
    zarr::store_chunk_i32(&idx_roi_ids, &[0], &roi_ids)?;
    let idx_bg_roi_ids = zarr::create_array_i32(
        &bg_store,
        schema::roi_ids_path(),
        vec![roi_ids.len() as u64],
        vec![roi_ids.len() as u64],
    ...
    let idx_bbox = zarr::create_array_i32(
        &roi_store,
        schema::roi_bboxes_path(),
        vec![roi_ids.len() as u64, n_times as u64, 4],
        vec![roi_ids.len() as u64, n_times as u64, 4],
    )?;
    ...
    let idx_present = zarr::create_array_u8(
        &roi_store,
        schema::roi_present_path(),
        vec![roi_ids.len() as u64, n_times as u64],
        vec![roi_ids.len() as u64, n_times as u64],
    )?;
    ...
    for bb in &bboxes {
        ...
        let raw_chunks = schema::raw_chunks(&raw_shape);
        let raw_shards = schema::raw_shards(&raw_shape);
        ...
        let arr = zarr::create_array_u16(
            &roi_store, &raw_path, raw_shape, raw_chunks, raw_shards, attrs,
        )?;
        ...
        if args.background {
            ...
            let bg_chunks = schema::bg_chunks(&bg_shape);
            let bg_shards = schema::bg_shards(&bg_shape);
    ...
    progress(
        1.0,
        &format!("Wrote {} and {}", roi_path.display(), bg_path.display()),
    );
    Ok(())
}
